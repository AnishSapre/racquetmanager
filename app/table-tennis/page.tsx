...
const SquashPage: React.FC = () => {
  ...
  const handleScoreIncrement = async (teamScored: "A" | "B") => {
    ...
    // Determine next server based on table tennis rules
    const totalPoints = nextScoreA + nextScoreB;
    const isDeuce = nextScoreA >= 10 && nextScoreB >= 10;
    
    // Service changes every 2 points normally, or every point at deuce
    const shouldChangeService = isDeuce ? true : (totalPoints % 2 === 0);
    
    if (shouldChangeService) {
      // Switch serving team
      nextServingTeam = servingTeam === "A" ? "B" : "A";
      // In doubles, either player can serve - no fixed server
      if (matchType === 'doubles') {
        const teamPositions = nextServingTeam === 'A' ? nextPlayerPositionsA : nextPlayerPositionsB;
        if (teamPositions) {
          // Randomly choose between the two players
          nextServerIndex = Math.random() < 0.5 ? teamPositions[0] : teamPositions[1];
        }
      } else {
        nextServerIndex = 0; // Only one player in singles
      }
    } else {
      // Keep same serving team
    ...
    if ((nextScoreA >= scoreToWin || nextScoreB >= scoreToWin) && Math.abs(nextScoreA - nextScoreB) >= 2) {
      ...
      if (nextSetsWon.A >= 3 || nextSetsWon.B >= 3) {
        ...
        if (matchType === 'doubles') {
          // Randomly choose between the two players
          nextServerIndex = Math.random() < 0.5 ? nextPlayerPositionsA[0] : nextPlayerPositionsA[1];
        } else {
          nextServerIndex = 0;
        }
      }
  ...
  return (
    <div className="h-screen flex flex-col bg-black text-white font-sans">
      ...
      <div className="pt-2 pb-1 text-center text-sm md:text-base">
        ...
        {servingTeam && <p>Serving: Team {servingTeam} from {getServerCourtPosition()} court</p>}
      </div>
      ...
      <div className="flex-1 flex justify-center items-center relative px-2 py-2">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
            {/* Court Lines */}
            <rect x="0" y="0" width="100%" height="100%" fill="rgb(100,119,250)" stroke="white" strokeWidth="0.5%" />
            ...
            {playerInfo && (
              <>
                ...
                {/* Right Side Team */}
                <text x="75%" y="25%" fill={isServer(2) ? "yellow" : "white"} fontSize="20" textAnchor="middle" dominantBaseline="middle" fontWeight={isServer(2) ? "bold" : "normal"}>
                  {getPlayerNameForQuad(2)}
                  {isServer(2) ? ' (S)' : ''}
                </text>
                {matchType === 'doubles' && (
                  <text x="75%" y="75%" fill={isServer(3) ? "yellow" : "white"} fontSize="20" textAnchor="middle" dominantBaseline="middle" fontWeight={isServer(3) ? "bold" : "normal"}>
