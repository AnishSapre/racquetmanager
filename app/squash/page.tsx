...
const SquashPage: React.FC = () => {
  ...
  const [setScoresA, setSetScoresA] = useState<number[]>([0, 0, 0, 0, 0]); // 5 sets
  const [setScoresB, setSetScoresB] = useState<number[]>([0, 0, 0, 0, 0]); // 5 sets
  ...
  const fetchGameData = useCallback(async () => {
    ...
    try {
      ...
      const fetchedSetScoresA = Array.isArray(data.setScoresA) ? data.setScoresA : JSON.parse(data.setScoresA || "[0,0,0,0,0]");
      const fetchedSetScoresB = Array.isArray(data.setScoresB) ? data.setScoresB : JSON.parse(data.setScoresB || "[0,0,0,0,0]");
  ...
  const handleScoreIncrement = async (teamScored: "A" | "B") => {
    ...
    let shouldSwitchSides = false;

    // Determine next server and positions based on squash rules
    if (teamScored === servingTeam) { // Serving team scored
      nextServingTeam = servingTeam;
      if (matchType === 'doubles') {
        nextServerIndex = currentServerIndex; // Same server continues
      } else {
        nextServerIndex = 0; // Only one player in singles
      }
      shouldSwitchSides = true; // Sides change when serving team wins
    } else { // Receiving team scored (side out)
      nextServingTeam = teamScored;
      if (matchType === 'doubles') {
        // In doubles, alternate between teammates
        const currentTeamPositions = nextServingTeam === 'A' ? nextPlayerPositionsA : nextPlayerPositionsB;
        if (currentTeamPositions) {
          nextServerIndex = currentServerIndex === currentTeamPositions[0] ? currentTeamPositions[1] : currentTeamPositions[0];
        }
      } else {
        nextServerIndex = 0; // Only one player in singles
      }
      shouldSwitchSides = false; // Sides don't change on side out
    }

    // Check for set/game completion
    const scoreToWin = 11;
    if ((nextScoreA >= scoreToWin || nextScoreB >= scoreToWin) && Math.abs(nextScoreA - nextScoreB) >= 2) {
      setCompleted = true;
      ...
      // Check game end (best of 5 sets)
      if (nextSetsWon.A >= 3 || nextSetsWon.B >= 3) {
        gameComplete = true;
      } else {
        // Start next set
        ...
        if (matchType === 'doubles') {
          // In doubles, alternate between teammates
          nextServerIndex = winner === 'A' ? nextPlayerPositionsA[0] : nextPlayerPositionsB[0];
        } else {
          nextServerIndex = 0;
        }
      }
    ...
    if (gameComplete) {
      ...
      if (finalSetScoresA[nextSet] === 0 && finalSetScoresB[nextSet] === 0) {
        finalSetScoresA[currentSet] = nextScoreA;
    ...
    try {
      const pointPayload: Omit<PointData, 'id' | 'timestamp' | 'gameId'> & { gameId: string } = {
        gameId: gameId!,
        scoredBy: teamScored,
        scoreA: setCompleted ? 0 : nextScoreA,
        scoreB: setCompleted ? 0 : nextScoreB,
        setNumber: setCompleted ? nextSet : currentSet,
  ...
  const getServerCourtPosition = (): 'Left' | 'Right' | null => {
    ...
    // Squash Rule:
    // After winning a point, the server must switch service boxes
    // So if the score is even (0, 2, 4...), serve from Right
    // If the score is odd (1, 3, 5...), serve from Left
    if (serverScore % 2 === 0) {
      return 'Left';
    } else {
      return 'Right';
  ...
  const playerTeamsInQuads: (('A' | 'B' | null))[] = [null, null, null, null];

  if (playerInfo && playerPositionsA && playerPositionsB) {
    const posA = playerPositionsA;
    const posB = playerPositionsB;

    // In squash, teams stay together on their side
    if (matchType === 'doubles') {
      // Team A is always on the left side
      if (leftSideTeam === 'A') {
        // Top-left quadrant (0)
        playerIndicesInQuads[0] = posA[0];
        playerTeamsInQuads[0] = 'A';
        // Bottom-left quadrant (1)
        playerIndicesInQuads[1] = posA[1];
        playerTeamsInQuads[1] = 'A';
        // Top-right quadrant (2)
        playerIndicesInQuads[2] = posB[0];
        playerTeamsInQuads[2] = 'B';
        // Bottom-right quadrant (3)
        playerIndicesInQuads[3] = posB[1];
        playerTeamsInQuads[3] = 'B';
      } else {
        // Team B is on the left side
        // Top-left quadrant (0)
        playerIndicesInQuads[0] = posB[0];
        playerTeamsInQuads[0] = 'B';
        // Bottom-left quadrant (1)
        playerIndicesInQuads[1] = posB[1];
        playerTeamsInQuads[1] = 'B';
        // Top-right quadrant (2)
        playerIndicesInQuads[2] = posA[0];
        playerTeamsInQuads[2] = 'A';
        // Bottom-right quadrant (3)
        playerIndicesInQuads[3] = posA[1];
        playerTeamsInQuads[3] = 'A';
      }
    } else {
      // Singles - players stay on their side
      if (leftSideTeam === 'A') {
        // Left side
        playerIndicesInQuads[0] = 0;
        playerTeamsInQuads[0] = 'A';
        // Right side
        playerIndicesInQuads[2] = 0;
        playerTeamsInQuads[2] = 'B';
      } else {
        // Left side
        playerIndicesInQuads[0] = 0;
  ...
  const getPlayerNameForQuad = (quadIndex: number): string => {
    const team = playerTeamsInQuads[quadIndex];
    const index = playerIndicesInQuads[quadIndex];
    if (!playerInfo || team === null || index === null) return '';
    const players = team === 'A' ? playerInfo.playersA : playerInfo.playersB;
    return players?.[index] || '?';
  }
      ...
      <div className="flex-1 flex justify-center items-center relative px-2 py-2">
        ...
        <div className="w-full h-full max-w-4xl max-h-[70vh] aspect-[0.8] md:aspect-[1.2] flex justify-center items-center">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
            ...
            <rect x="0" y="0" width="100%" height="100%" fill="rgb(220,180,140)" stroke="red" strokeWidth="0.5%" />
            {/* Vertical court line */}
            <line
              x1="50%"
              y1="0%"
            ...
            {playerInfo && (
              <>
                {/* Left Side Team */}
                <text x="60%" y="15%" fill={isServer(0) ? "yellow" : "white"} fontSize="25" textAnchor="middle" dominantBaseline="middle" fontWeight={isServer(0) ? "bold" : "normal"}>
                  {getPlayerNameForQuad(0)}
                  {isServer(0) ? ' (S)' : ''}
                </text>
                {matchType === 'doubles' && (
                  <text x="60%" y="25%" fill={isServer(1) ? "yellow" : "white"} fontSize="25" textAnchor="middle" dominantBaseline="middle" fontWeight={isServer(1) ? "bold" : "normal"}>
                ...
                {/* Right Side Team */}
                <text x="60%" y="75%" fill={isServer(2) ? "yellow" : "white"} fontSize="20" textAnchor="middle" dominantBaseline="middle" fontWeight={isServer(2) ? "bold" : "normal"}>
                  {getPlayerNameForQuad(2)}
                  {isServer(2) ? ' (S)' : ''}
                </text>
                {matchType === 'doubles' && (
                  <text x="60%" y="85%" fill={isServer(3) ? "yellow" : "white"} fontSize="20" textAnchor="middle" dominantBaseline="middle" fontWeight={isServer(3) ? "bold" : "normal"}>
        ...
        {/* Left Score Button */}
        <button
          className="absolute right-20 top-3/4 transform -translate-y-1/2 text-white text-2xl md:text-4xl font-extrabold bg-gray-600 px-3 py-4 md:px-5 md:py-8 rounded-xl shadow-lg disabled:opacity-40 disabled:cursor-not-allowed z-10"
        ...
        {/* Right Score Button */}
        <button
          className="absolute right-20 top-1/4 transform -translate-y-1/2 text-white text-2xl md:text-4xl font-extrabold bg-gray-600 px-3 py-4 md:px-5 md:py-8 rounded-xl shadow-lg disabled:opacity-40 disabled:cursor-not-allowed z-10"
          ...
          <table className="w-full border border-gray-600 text-white text-sm md:text-base text-center bg-gray-800 rounded-lg shadow-md">
            <thead className="bg-gray-700">
              <tr>
                ...
                <th className="border border-gray-600 px-2 py-1 md:px-3 md:py-2">S4</th>
                <th className="border border-gray-600 px-2 py-1 md:px-3 md:py-2">S5</th>
            ...
            <tbody>
              <tr className={leftSideTeam === 'A' ? 'bg-blue-900 bg-opacity-50' : 'bg-red-900 bg-opacity-50'}>
                ...
                <td className="border border-gray-600 px-2 py-1 md:px-3 md:py-2">{setScoresA[3] > 0 || setScoresB[3] > 0 ? setScoresA[3] : '-'}</td>
                <td className="border border-gray-600 px-2 py-1 md:px-3 md:py-2">{setScoresA[4] > 0 || setScoresB[4] > 0 ? setScoresA[4] : '-'}</td>
              ...
              <tr className={leftSideTeam === 'B' ? 'bg-blue-900 bg-opacity-50' : 'bg-red-900 bg-opacity-50'}>
                ...
                <td className="border border-gray-600 px-2 py-1 md:px-3 md:py-2">{setScoresA[3] > 0 || setScoresB[3] > 0 ? setScoresB[3] : '-'}</td>
                <td className="border border-gray-600 px-2 py-1 md:px-3 md:py-2">{setScoresA[4] > 0 || setScoresB[4] > 0 ? setScoresB[4] : '-'}</td>
